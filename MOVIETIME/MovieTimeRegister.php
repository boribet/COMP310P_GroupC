...
if(isset($_POST['btn-signup'])) {
 ...
 $upass2= strip_tags($_POST['confirm_password']);
 $hashPass = password_hash($upass, PASSWORD_DEFAULT);

 mysqli_real_escape_string($con, $uname);
 mysqli_real_escape_string($con, $email);
 mysqli_real_escape_string($con, $hashPass);
 mysqli_real_escape_string($con, $f_name);
 mysqli_real_escape_string($con, $l_name);
...
if (!($upass==$upass2)) {
  ...
  values('$uname','$hashPass','$email','$f_name','$l_name')";
  if ($con->query($query)) {
...
<html>
<head>
	<title>Register to MovieTime</title>
...
<div class="form-group">
  <label class="col-md-4 control-label" >Password</label> 
    ...
    <div class="input-group">
  <span class="input-group-addon"><i class="glyphicon glyphicon-user"></i></span>
  <input name="password" placeholder="Password" class="form-control"  type="password" required maxlength="15" minlength="5">

    </div>
     <small id="emailHelp" class="form-text text-muted"> Your password must be 5 to 15 characters.</small>
  </div>
</div>
...
<div class="form-group">
  ...
  <div class="col-md-4"><br>
    <button type="submit" class="btn btn-danger" name="btn-signup">Register<span class="glyphicon glyphicon-send"></span></button>
  </div>
  <a href="MovieTimeIndex.php" class="btn btn-default" style="float:right;">Back to login</a>
